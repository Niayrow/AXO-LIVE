  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-4 right-4 w-12 h-12 bg-slate-950/40 backdrop-blur-xl border border-white/10 rounded-full flex items-center justify-center text-amber-500 shadow-[0_8px_32px_rgba(0,0,0,0.3)] hover:scale-110 active:scale-95 transition-all duration-300 z-50 cursor-pointer ${isCollapsed ? "translate-y-0 opacity-100 scale-100" : "translate-y-24 opacity-0 scale-95 pointer-events-none"
          }`}
      ...
      >
        ...
        <nav className="relative h-16 w-full bg-slate-950/40 backdrop-blur-xl border border-white/10 rounded-2xl px-6 flex justify-between items-center shadow-[0_12px_40px_rgba(0,0,0,0.35)] pointer-events-auto">
 
          {/* Inner Minimize Button */}
          {/* OPTIMISATION TACTILE : Zone de clic agrandie à h-8 w-8 et centrée pour les doigts */}
          <button
            onClick={() => handleCollapse(true)}
            className="absolute top-0.5 right-1 w-8 h-8 flex items-center justify-center text-slate-300 hover:text-amber-500 transition-colors z-20 cursor-pointer rounded-full hover:bg-white/5"
                ...
                <div key={item.href} className="relative flex flex-col items-center justify-center w-full h-full">
                  <Link
                    ...
                    className={`flex items-center justify-center w-12 h-12 rounded-full border transition-all duration-300 hover:scale-105 active:scale-90 shadow-lg backdrop-blur-md -mt-7 ${isActive
                        ...
                        : "bg-slate-900/40 border-white/10 text-amber-500 hover:border-amber-500/50 hover:bg-slate-900 shadow-[0_4px_12px_rgba(0,0,0,0.3)]"
                      }`}
                  >
                    <Icon size={22} className="stroke-[2.5] transition-transform duration-300 hover:rotate-6" />
                  </Link>
                  <span className={`text-[8px] font-black uppercase tracking-widest mt-1 transition-colors ${isActive ? "text-amber-500" : "text-slate-200"
                    }`}>
                    {item.name}
                  </span>
                </div>
